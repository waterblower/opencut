...
fn shared_timeline_round_trip_and_backend_rendering() {
    ...
    let options = ExportOptions::from_timeline(&edited);
    let ges_timeline = build_ges_timeline(&edited, &root, options).unwrap();
    ...
    pipeline
        .set_render_settings(
            Url::from_file_path(&gstreamer_output).unwrap().as_str(),
            &profile,
        )
        .unwrap();
    ...
    let result = render_pipeline(
        &pipeline,
        edited.duration(edited.content_duration()),
        &mut |_| {},
    );
